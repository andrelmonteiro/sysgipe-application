/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import ArquivoDetailComponent from '@/entities/arquivo/arquivo-details.vue';
import ArquivoClass from '@/entities/arquivo/arquivo-details.component';
import ArquivoService from '@/entities/arquivo/arquivo.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('Arquivo Management Detail Component', () => {
    let wrapper: Wrapper<ArquivoClass>;
    let comp: ArquivoClass;
    let arquivoServiceStub: SinonStubbedInstance<ArquivoService>;

    beforeEach(() => {
      arquivoServiceStub = sinon.createStubInstance<ArquivoService>(ArquivoService);

      wrapper = shallowMount<ArquivoClass>(ArquivoDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { arquivoService: () => arquivoServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundArquivo = { id: 123 };
        arquivoServiceStub.find.resolves(foundArquivo);

        // WHEN
        comp.retrieveArquivo(123);
        await comp.$nextTick();

        // THEN
        expect(comp.arquivo).toBe(foundArquivo);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundArquivo = { id: 123 };
        arquivoServiceStub.find.resolves(foundArquivo);

        // WHEN
        comp.beforeRouteEnter({ params: { arquivoId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.arquivo).toBe(foundArquivo);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
